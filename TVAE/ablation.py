...
def knn_analysis_per_class(
    ...
    for cls in np.unique(y_under):
        ...
        df_under = pd.DataFrame(X_cls, columns=feature_names)
        ...
        # df_combo = pd.concat([df_under, df_neigh], ignore_index=True)
        # df_combo = df_under
...
def main():
    ...
    for cls, df_cls in per_class.items():
        ...
        print("Generator Training shape", str(df_cls.shape))
        print("Columns in df_cls:", df_cls.columns)
    ...
    '''df_all = pd.concat([df_cls.assign(Label=cls) for cls, df_cls in per_class.items()], ignore_index=True)
    if 'Label' not in discrete:
        discrete.append('Label')
    tvae = TVAE(epochs=50,
                    batch_size=60,
                    )
    print("Generator Training shape", str(df_all.shape))
    tvae.fit(df_all, discrete)
    n_samples = int(df_all.shape[0] * args.num_synthetic)
    synthetic_df = tvae.sample(samples=n_samples)'''
    ...
    fname = f"{args.method}+TVAE+ALFA.txt"
