...
def compute_anchor_fraction(f_c, min_frac=0.01, alpha=1, steepness=100):
    raw = alpha*math.exp(-(steepness * (f_c - min_frac)))
    return raw
...
def main():
    ...
    if args.method == "galaxy" or args.method == "clue":
        ...
        clf = XGBClassifier(use_label_encoder=False, eval_metric='mlogloss', random_state=args.random_state)
    ...
    for u, f in zip(uniq, freqs):
        ...
        print("bear",u, round(f, 2),round(frac, 2))
        n = int(frac * len(idx))
        print(n)
    ...
    for cls, df_cls in per_class.items():
        print("Generator training size", df_cls.shape[0])
        tvae = TVAE(epochs=50,
                    batch_size=60,
                    )
