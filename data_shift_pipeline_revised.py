...
def _define_minority_classes(
    ...
    Returns:
      ...
      majority_set: set of class labels considered majority
      minority_set: set of class labels considered minority
    """
    ...
    if minority_def == "pct_total":
        # Minority classes are those whose overall frequency is below a percentage of the total dataset.
        # Example: threshold=0.05 => any class with <5% of all rows is minority.
        if total <= 0:
            minority = set()
        ...
        # bottom percentile of counts are minority
        n = len(classes)
    ...
    if len(minority) == 0 and len(majority) > 1:
        # Make the smallest class minority
...
def split_domain_shift_class_imbalance(
    ...
    """
    Creates a label-distribution drift tailored for ALFA evaluation:

      - Domain A (train / labeled): majority-heavy, minority-scarce
      - Domain B (val+test / unlabeled+test): minority-heavy

    Returns:
      df_train, df_val, df_test, shift_meta (dict)
    """
    rng = np.random.default_rng(random_state)
    ...
    # Train size is determined first (priority: shape Domain A composition)
    ...
    if (not allow_replacement) and n_train >= n_total:
        # need at least 2 samples remaining to form val/test without replacement
        n_train = max(1, n_total - 2)

    remaining = n_total - n_train
    n_future = remaining
    if remaining < 2 and not allow_replacement:
        raise ValueError(
    ...
    # Target label composition
    train_min_n = int(round(n_train * float(labeled_minority_frac)))
    train_maj_n = n_train - train_min_n

    future_min_n = int(round(n_future * float(domainb_minority_frac)))
    future_maj_n = n_future - future_min_n
    ...
    train_idx = np.concatenate([train_min, train_maj])
    ...
    if class_shift_policy == "strict" and len(train_idx) != n_train:
        raise RuntimeError(
            f"STRICT class_imbalance: could not build train of size {n_train}. Got {len(train_idx)}. "
            f"Try --class_shift_policy soft or --allow_replacement, or adjust fractions."
        )

    if class_shift_policy == "soft" and len(train_idx) < n_train:
        remaining = np.setdiff1d(idx_all, train_idx, assume_unique=False)
        need = n_train - len(train_idx)
        if len(remaining) > 0:
            filler = rng.choice(remaining, size=min(need, len(remaining)), replace=allow_replacement)
            train_idx = np.concatenate([train_idx, filler])

    train_idx = np.unique(train_idx)

    # --- Domain B (future = val+test) ---
    future_min = _take(min_pool, future_min_n)
    future_maj = _take(maj_pool, future_maj_n)
    future_idx = np.concatenate([future_min, future_maj])

    if not allow_replacement:
        # ensure no overlap with train
        future_idx = np.setdiff1d(future_idx, train_idx, assume_unique=False)

    if class_shift_policy == "strict" and len(future_idx) != n_future:
        raise RuntimeError(
    ...
    if class_shift_policy == "soft" and len(future_idx) < n_future:
        remaining = np.setdiff1d(idx_all, np.unique(np.concatenate([train_idx, future_idx])), assume_unique=False)
        need = n_future - len(future_idx)
        if len(remaining) > 0:
            filler = rng.choice(remaining, size=min(need, len(remaining)), replace=allow_replacement)
            future_idx = np.concatenate([future_idx, filler])

    future_idx = np.unique(future_idx)

    # Split future into val/test (keep random mix inside Domain B)
    df_future = df_clean.iloc[future_idx].copy()
    # Decide how much of Domain B becomes validation vs test.
    # This is a simple split of the remaining pool; you can control it with val_frac_of_remaining.
    ...
    df_val, df_test = train_test_split(
        df_future,
        test_size=(n_test / max(n_future, 1)),
        random_state=random_state,
        shuffle=True,
        stratify=df_future[label_col] if df_future[label_col].nunique() > 1 else None
    )
...
def main():
    ...
    # General split proportions (used for shift_mode none/cluster/quantile). Not fixed 50/25/25.
    parser.add_argument('--train_frac', type=float, default=0.7, help="Fraction of rows used for train.")
    parser.add_argument('--val_frac', type=float, default=0.15, help="Fraction of rows used for validation.")
    parser.add_argument('--test_frac', type=float, default=0.15, help="Fraction of rows used for test.")
    ...
    # class-imbalance shift params (Domain A vs Domain B label distribution)
    ...
    parser.add_argument('--val_frac_of_remaining', type=float, default=0.5,
                        help="For class_imbalance: fraction of remaining (Domain B) assigned to validation; rest to test.")
    parser.add_argument('--labeled_minority_frac', type=float, default=0.02,
    ...
    if args.unlabeled_csv is not False:
        ...
        if args.shift_mode == "none":
            # Flexible split proportions (not fixed 50/25/25).
    ...
    # Also save a flat CSV for easy plotting
    dist_rows = []
