...
def main():
    ...
    parser.add_argument("--num_synthetic", type=float, default=1)
    ...
    parser.add_argument("--steepness", type=float, default=100, help="Seed for reproducibility")
    ...
    for cls, df_cls in per_class.items():
        ...
        ctgan.fit(df_cls, discrete)
        # sample proportional to class_df size
        n_samples = int((count_per_cat[cls]) * args.num_synthetic)
        df_syn = ctgan.sample(n=n_samples)
        df_syn = pd.concat([df_syn, pd.Series(cls, index=df_syn.index, name='Label')], axis=1)
    ...
    fname = f"ctgan_{script_name}_{args.dataset}_{args.method}_{args.budget}.txt"
    path = os.path.join('results', fname)

    with open(path, 'w') as f:
        f.write(f"Time: {exec_time:.2f}\n")
